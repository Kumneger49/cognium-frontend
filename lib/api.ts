...
export async function fetchNews(): Promise<NewsItem[]> {
	...
	const res = await fetch(`https://api.cognium.xyz/api/recommendations/client/client_123`);
...
export async function regenerateRecommendations(): Promise<{ success: boolean; message?: string }> {
	try {
		const res = await fetch(`https://api.cognium.xyz/api/regenerate-recommendations/client/client_123`, {
			method: 'POST',
			headers: {
